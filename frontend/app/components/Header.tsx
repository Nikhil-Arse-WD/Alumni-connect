...
export default function Header() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      getUserId().then(id => {
        if (id) socket.emit("register_user", id.toString());
      });

      const onNewNotification = () => fetchNotifCount();
      const onNewForumPost = () => { if (!forumVisited.current) fetchForumCount(); };

      socket.on("new_notification", onNewNotification);
      socket.on("new_forum_post", onNewForumPost);

      // Cleanup listeners
      return () => {
        socket.off("new_notification", onNewNotification);
        socket.off("new_forum_post", onNewForumPost);
      };
    }, [])
  );
