...
export default function AdminJobs() {
  ...
  const handleClose = async (id: number) => {
    try {
      await axios.put(`${API}/jobs/close/${id}`);
      if (Platform.OS === "web") window.alert("✅ Job closed successfully");
      else Alert.alert("Done ✅", "Job closed");
      fetchJobs();
    } catch (err) {
      if (Platform.OS === "web") window.alert("❌ Failed to close job");
      else Alert.alert("Error", "Failed to close job");
    }
  };

  const handleFlag = async (id: number, flag: number) => {
    try {
      await axios.put(`${API}/admin/jobs/flag/${id}`, { is_flagged: flag });
      if (Platform.OS === "web") window.alert(flag ? "🚩 Job flagged" : "✅ Flag removed");
      else Alert.alert("Done ✅", flag ? "Job flagged" : "Flag removed");
      fetchJobs();
    } catch (err) {
    ...
    if (Platform.OS === "web") {
      ...
      if (ok) {
        try {
          ...
          fetchJobs();
        } catch (err) { window.alert("❌ Failed to delete job"); }
      }
      return;
    }
      ...
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            fetchJobs();
          } catch (err) { Alert.alert("Error", "Failed to delete job"); }
        },
      },
    ]);
  };
      ...
      {isWeb ? (
        <View style={{ flex: 1, flexDirection: "row" }}>
          ...
          <FlatList
            ...
            contentContainerStyle={[styles.listContent, { maxWidth: 1150, alignSelf: "center", width: "100%" }]}
            ListHeaderComponent={ListHeader}
            renderItem={renderJob}
          />
        </View>
      ) : (
        /* ── MOBILE: topbar + drawer + list ── */
        <>
