...
// Map to store connected users (alumni_id -> socket_id)
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", () => {
    ...
    for (const [alumni_id, id] of userSockets.entries()) {
      if (id === socket.id) {
        userSockets.delete(alumni_id);
        break;
      }
    }
...
app.set("io", io); // optional, if we need it in routes
...
if (!fs.existsSync(path.join(UPLOADS_DIR, "events"))) fs.mkdirSync(path.join(UPLOADS_DIR, "events"), { recursive: true });
if (!fs.existsSync(path.join(UPLOADS_DIR, "event-gallery"))) fs.mkdirSync(path.join(UPLOADS_DIR, "event-gallery"), { recursive: true });
if (!fs.existsSync(path.join(UPLOADS_DIR, "banners"))) fs.mkdirSync(path.join(UPLOADS_DIR, "banners"), { recursive: true });
...
const galleryStorage = multer.diskStorage({
  destination: (req, file, cb) => cb(null, path.join(UPLOADS_DIR, "event-gallery")),
  filename: (req, file, cb) => {
    const uniqueSuffix = Date.now() + "-" + Math.round(Math.random() * 1e9);
    cb(null, uniqueSuffix + path.extname(file.originalname));
  },
...
const sendNotification = (alumni_id, title, message, type = "general") => {
  db.query(
    ...
    (err) => {
      ...
      const socketId = userSockets.get(alumni_id.toString());
      if (socketId) {
        io.to(socketId).emit("new_notification", { title, message, type });
      }
    }
  );
};
