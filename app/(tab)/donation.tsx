...
function UserAvatar({ photo, name, size = 64 }: { photo?: string | null; name?: string; size?: number }) {
  ...
  if (photo) {
    return (
      <Image
        source={{ uri: `${API_BASE}/uploads/${photo}` }}
        style={{ width: size, height: size, borderRadius: size / 2, borderWidth: 2, borderColor: "#E2E8F0" }}
      />
    );
  }
  return (
...
function CommunityCard({ item }: { item: any }) {
  ...
  const donationIcon =
    item.donation_type === "Money" ? "💰" :
    item.donation_type === "Equipment" ? "🖥️" : "🎓";
    ...
    <View style={cStyles.card}>
      <View style={cStyles.cardHeader}>
        ...
        <View style={{ flex: 1, marginLeft: 14 }}>
          ...
          <Text style={cStyles.designation} numberOfLines={1}>
            {item.designation || donationLabel}
          </Text>
        </View>
      </View>
      ...
      {(item.programme || item.batch_year) && (
        <View style={cStyles.pillRow}>
          {item.programme && (
            <View style={cStyles.pillCourse}>
              <Text style={cStyles.pillCourseText}>{item.programme}</Text>
            </View>
          )}
          {item.batch_year && (
            <View style={cStyles.pillBatch}>
              <Text style={cStyles.pillBatchText}>{item.batch_year}</Text>
            </View>
          )}
        </View>
      )}
      ...
      <View style={cStyles.donationBox}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {item.amount && (
            <Text style={cStyles.donationAmount}>₹{Number(item.amount).toLocaleString("en-IN")}</Text>
          )}
          {item.equipment_description && (
            <Text style={cStyles.donationDesc} numberOfLines={2}>{item.equipment_description}</Text>
          )}
          {item.scholarship_description && (
            <Text style={cStyles.donationDesc} numberOfLines={2}>{item.scholarship_description}</Text>
          )}
          {item.message && (
            <Text style={cStyles.donationDesc} numberOfLines={2}>"{item.message}"</Text>
          )}
        </View>
...
function MyContribCard({ item, type }: { item: any; type: "lecture" | "mentor" | "donation" }) {
  ...
  const title =
    type === "lecture" ? item.topic || item.title :
    type === "mentor"  ? item.expertise :
    item.donation_type === "Money" ? `₹${item.amount}` :
    item.donation_type || "Donation";

  const sub =
    type === "lecture" ? `Mode: ${item.mode || "—"} · ${item.created_at?.split("T")[0]}` :
    type === "mentor"  ? `Max ${item.max_mentees} mentees` :
    item.receipt_number ? `🧾 ${item.receipt_number}` : item.created_at?.split("T")[0];

  return (
    <View style={mcStyles.card}>
      <View style={mcStyles.left}>
        <View style={mcStyles.iconBox}>
...
// ── INTEGRATED PAYMENT GATEWAY FLOW ──
    ...
    try {
      ...
      if (donationType === "Money" && res.data.donation_id) {
        const initRes = await axios.post(`${API_BASE}/pay/initiate`, {
          ...
          firstname: user.full_name.trim(),
          email: user.email.trim(),
          phone: user.mobile ? user.mobile.replace(/[\s\-\+]/g, "") : "9999999999", 
        ...
        if (initRes.data && initRes.data.checkout_url) {
          if (Platform.OS === "web") {
            window.alert(`Donation Initiated! Redirecting to Easebuzz payment gateway...`);
            window.location.href = initRes.data.checkout_url;
          } else {
            await Linking.openURL(initRes.data.checkout_url);
            
            // Hold the user in the app with an inescapable alert
            Alert.alert(
              "Payment Verification",
              "Please complete your donation in the browser window.\n\nIf your transaction failed or was cancelled, click below to try again.",
              [
                { 
                  text: "❌ Payment Failed / Retry", 
                  style: "cancel",
                  onPress: () => { 
                    // Instantly resets the button so they can generate a new link
                    setIsSubmitting(false); 
                    fetchMyData(user.id); 
                  } 
                },
                { 
                  text: "✅ I Have Paid", 
                  onPress: () => {
                    // Clears the form inputs on success
                    setIsSubmitting(false);
              ...
              { cancelable: false } // <--- PREVENTS TAPPING OUTSIDE THE BOX
            );
          }
          return; // Halts function so it waits for the user's Alert choice
        }
      }
              ...
              <>
                <View style={styles.card}>
                  ...
                  <View style={styles.chipRow}>
                    {BATCH_OPTIONS.map(b => (
                      <TouchableOpacity key={b} style={[styles.chip, selectedBatches.includes(b) && styles.chipActive]} onPress={() => toggleChip(b, selectedBatches, setSelectedBatches)}>
                  ...
                  <TouchableOpacity style={[styles.submitBtn, isSubmitting && styles.submitBtnDisabled]} onPress={submitLecture} disabled={isSubmitting}>
              ...
              <View style={styles.card}>
                ...
                {myMentor && (
                  <View style={[styles.statusBanner, { backgroundColor: statusColor(myMentor.status).bg }]}>
                    <Text style={{ color: statusColor(myMentor.status).text, fontWeight: "700" }}>Current Status: {myMentor.status}</Text>
                    <Text style={{ color: statusColor(myMentor.status).text, fontSize: 12, marginTop: 2 }}>Expertise: {myMentor.expertise}</Text>
                  </View>
                )}
                <Text style={styles.fieldLabel}>Areas of Expertise *</Text>
                <View style={styles.chipRow}>
                  {EXPERTISE_OPTIONS.map(e => (
                    <TouchableOpacity key={e} style={[styles.chip, selectedExpertise.includes(e) && styles.chipActive]} onPress={() => toggleChip(e, selectedExpertise, setSelectedExpertise)}>
                ...
                <View style={styles.chipRow}>
                  {["1", "2", "3", "5"].map(n => (
                    <TouchableOpacity key={n} style={[styles.chip, maxMentees === n && styles.chipActive]} onPress={() => setMaxMentees(n)}>
                      <Text style={[styles.chipText, maxMentees === n && styles.chipActiveText]}>{n}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
                <TouchableOpacity style={[styles.submitBtn, isSubmitting && styles.submitBtnDisabled]} onPress={submitMentor} disabled={isSubmitting}>
                ...
                <View style={styles.card}>
                  ...
                  <View style={styles.chipRow}>
                    {["Money", "Equipment", "Scholarship"].map(t => (
                      <TouchableOpacity key={t} style={[styles.chip, donationType === t && styles.chipActive]} onPress={() => setDonationType(t)}>
                    ...
                    <>
                      ...
                      <View style={styles.chipRow}>
                        {DONATION_AMOUNTS.map(a => (
                          <TouchableOpacity key={a} style={[styles.chip, selectedAmount === a && styles.chipActive]} onPress={() => { setSelectedAmount(a); setCustomAmount(""); }}>
              ...
              <>
                ...
                {myLectures.length === 0 && !myMentor && myDonations.length === 0 ? (
                  ...
                  <View style={cStyles.grid}>
                    {myLectures.map(item => (
                      <View key={item.id} style={cStyles.gridItem}>
                        <MyContribCard item={item} type="lecture" />
                      </View>
                    ))}
                    {myMentor && (
                      <View style={cStyles.gridItem}>
                        <MyContribCard item={myMentor} type="mentor" />
                      </View>
                    )}
                    {myDonations.map(item => (
                      <View key={item.id} style={cStyles.gridItem}>
                        <MyContribCard item={item} type="donation" />
                      </View>
                    ))}
                  </View>
                )}
...
const cStyles = StyleSheet.create({
  ...
  gridItem: { flex: 1, minWidth: 280 }, // Replaced calc() with native flexbox wrapping for flawless web/mobile scaling
  ...
  donationBox: { flexDirection: "row", alignItems: "flex-start", gap: 10, backgroundColor: "#FFFBEB", borderRadius: 12, padding: 12, marginBottom: 16, borderWidth: 1, borderColor: "#FDE68A" },
  donationIcon: { fontSize: 20 },
  donationLabel: { fontSize: 13, fontWeight: "700", color: "#92400E", marginBottom: 3 },
  donationAmount: { fontSize: 16, fontWeight: "900", color: "#16A34A", marginBottom: 2 },
  donationDesc: { fontSize: 12, color: "#78350F", lineHeight: 17 },
  donationDate: { fontSize: 10, color: "#A16207", fontWeight: "600", marginTop: 2 },
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: "#F1F5F9", marginBottom: 63 },
  webContainer: { maxWidth: 900, alignSelf: "center", width: "100%" }, // Centers and restricts width on Desktop browsers
  hero: { paddingHorizontal: 18, paddingTop: 20, paddingBottom: 28 },
  ...
  card: { backgroundColor: "#fff", borderRadius: 22, padding: 18, marginBottom: 14, elevation: 3, shadowColor: "#000", shadowOpacity: 0.07, shadowRadius: 10 },
  ...
  input: { backgroundColor: "#F8FAFC", borderWidth: 1, borderColor: "#E2E8F0", borderRadius: 14, paddingHorizontal: 14, height: 50, fontSize: 14, color: "#111", marginBottom: 12 },
  ...
  submitBtn: { backgroundColor: "#4F46E5", paddingVertical: 15, borderRadius: 16, alignItems: "center", marginTop: 6 },
